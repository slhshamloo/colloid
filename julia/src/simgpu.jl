...
function apply_parallel_step!(cusim::CuColloidSim, cell_list::CuCellList, 
    ...
    group_potentials = CuDynamicSharedArray(eltype(cusim.colloid.centers),
...
function apply_parallel_move!(cusim::CuColloidSim, cell_list::CuCellList,
    ...
    if isnothing(cusim.pairpotential)
        checkoverlap!(cusim.colloid, cell_list, cusim.constraints, idx, group_potentials,
                      maxcount, group, thread, i, j, is_thread_active)
        CUDA.sync_threads()
    elseif groupcount > 9 * maxcount
        checkconstraint!(cusim.colloid, cusim.constraints, idx, group_potentials,
    ...
    if is_thread_active && iszero(group_potentials[group])
        ...
        if constraint_index > 0
            passed_fast_check, dist, distnorm = constraint_step_one!(
                colloid, constraints, group_potentials, idx[group], constraint_index, group)
        else
            passed_circumference_check, centerangle, neighbor, dist, distnorm = 
                overlap_step_one!(colloid, cell_list, group_potentials, idx[group],
